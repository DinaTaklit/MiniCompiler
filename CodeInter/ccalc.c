...
void DisplayLine(void) {
  if (  NmLine != 0  ) 
   fprintf(stdout, "%6d |%.*s", NmLine, BufferSize, buffer);
}
//====================================================================
...
int getNextLine(void) {
  ...
  p = fgets(buffer, MaxBuff, fileinput);
  if (  p == NULL  ) {
    if (  ferror(fileinput)  )
      return -1;
    eof = true;
...
void BeginToken(char *t) {
  ...
  /* location for bison --------------------------------------------*/
  yylloc.first_line = NmLine;
  yylloc.first_column = StartToken;
  yylloc.last_line = NmLine;
...
/*extern
int main() {
 printf("     === Sample Calculator Intermediate Code ===     \n\n");
